...
export default function AffiliatePortal() {
  ...
  return <AffiliateLayout portal><Page title={t("dashboard")}>
    {!data && !error && <Loading />}{error && <ErrorState />}{data && <>
      ...
      <h2 className="mb-3 mt-8 text-xl font-bold">KPIs</h2><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4"><Metric label={t("linkClicks")} value={data.metrics.clicks} /><Metric label={t("recommendations")} value={data.metrics.referrals} /><Metric label={t("contracts")} value={data.metrics.contracts} /><Metric label={t("confirmed")} value={data.metrics.confirmedContracts} /></div>
      <h2 className="mb-3 mt-8 text-xl font-bold">{t("rewards")}</h2><div className="grid gap-4 sm:grid-cols-3"><Metric label={t("pending")} value={data.rewards.pending} currency={data.rewards.currency} /><Metric label={t("available")} value={data.rewards.available} currency={data.rewards.currency} /><Metric label={t("paid")} value={data.rewards.paid} currency={data.rewards.currency} /></div>
    </>}
...
function Metric({ label, value, currency }: { label: string; value: number; currency?: string }) { return <Card><CardContent className="pt-6"><p className="text-sm text-muted-foreground">{label}</p><p className="mt-2 text-2xl font-bold">{currency ? new Intl.NumberFormat("de-DE", { style: "currency", currency }).format(value) : value.toLocaleString("de-DE")}</p></CardContent></Card>; }
