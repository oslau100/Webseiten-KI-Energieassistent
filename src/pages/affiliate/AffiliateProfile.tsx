...
export default function AffiliateProfilePage() {
  ...
  return <AffiliateLayout portal><Page title={t("profile")}>{!data && !error && <Loading />}{error && <ErrorState />}{data && <div className="grid gap-5 lg:grid-cols-2">
    <Info icon={UserRound} title={t("accountInfo")}><p className="font-medium">{data.profile.name}</p><p className="mt-1 text-sm text-muted-foreground">{data.profile.email}</p></Info>
    <Info icon={CreditCard} title={t("payout")}>{data.method.maskedIban ? <><p className="font-medium" dir="ltr">{data.method.maskedIban}</p><p className="mt-1 text-sm text-muted-foreground">{data.method.accountHolder}</p></> : <Empty text={t("payoutMissing")} />}</Info>
