...
export function BookingContactForm({value,onChange,onSubmit,submitting,errors,b,privacyPath,onBack}:Props){
 ...
 return <form className="space-y-5" noValidate onSubmit={(event)=>{event.preventDefault();onSubmit();}}>
  ...
  <div className="grid gap-4 sm:grid-cols-2">{field("email",b.email,"email","email",254)}<div><Label htmlFor="phone">{b.phone}</Label><div className="relative" dir="ltr"><span className="pointer-events-none absolute inset-y-0 left-3 z-10 flex items-center gap-2 font-medium" aria-hidden="true"><span className="text-lg">🇩🇪</span><span>+49</span></span><Input id="phone" name="phone" type="tel" inputMode="tel" value={value.phone} maxLength={40} autoComplete="tel-national" className="ps-[5.75rem]" aria-invalid={Boolean(errors.phone)} aria-describedby={errors.phone?"phone-error":undefined} onChange={(event)=>onChange({...value,phone:event.target.value.replace(/^\s*(?:\+49|0049)\s*/,"")})}/></div>{errors.phone&&<p id="phone-error" className="mt-1 text-sm text-destructive">{errors.phone}</p>}</div></div>
  ...
  <div><div className="flex items-start gap-3"><Checkbox id="consent" checked={value.consent} aria-invalid={Boolean(errors.consent)} aria-describedby={errors.consent?"consent-error":undefined} onCheckedChange={(checked)=>onChange({...value,consent:checked===true})}/><Label htmlFor="consent" className="leading-relaxed">{b.consent} <Link className="text-primary underline" target="_blank" rel="noopener noreferrer" to={privacyPath}>{b.privacyLink}<span className="sr-only"> ({b.privacyNewTab})</span></Link></Label></div>{errors.consent&&<p id="consent-error" className="mt-1 text-sm text-destructive">{errors.consent}</p>}</div>
