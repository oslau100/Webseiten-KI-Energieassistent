...
const INITIAL_WIDGET_HEIGHT = 320;

type GetReviewWidgetProps = { widgetId?: string | null; className?: string };
...
/** Isolates the external provider from the page document and controls its height. */
export const GetReviewWidget = ({ widgetId, className = "" }: GetReviewWidgetProps) => {
  ...
  useEffect(() => {
    const onMessage = (event: MessageEvent<unknown>) => {
      ...
      setHeight(Math.max(INITIAL_WIDGET_HEIGHT, Math.ceil(data.height)));
    };
  ...
  return (
    <div className={`w-full max-w-full min-w-0 overflow-hidden ${className}`.trim()} data-no-translate="true" data-testid="get-review-widget">
      <iframe
        ...
        data-testid="get-review-widget-main"
